...
public class PlayerActivity extends AppCompatActivity
    ...
    // Activity lifecycle
    private Subscription mPlayerObservable;
...
//              /* context= */ this, renderersFactory, trackSelector, drmSessionManager);
                            this, renderersFactory, trackSelector, new DefaultLoadControl.Builder()
        ...
        mPlayerObservable = Observable
                ...
                .subscribe(t -> {
                    runOnUiThread(() -> {
                        ...
                        if (buffered - current > 3500) {
//                            lastSeekAt = System.currentTimeMillis();

                            player.seekTo(player.getBufferedPosition() - 500);
                            DashMediaSource source = (DashMediaSource) player.getMediaSource();

                            Log.d("lalala", "(" + source.getManifestUri() + ")Seeking from current - " + current + " to buffered - " + buffered);
                        }
                    });
                }, e -> {
                    mPlayerObservable = null;
                    e.printStackTrace();
                });
    }
    ...
    private MediaSource buildMediaSource(Uri uri, @Nullable String overrideExtension) {
        ...
        switch (type) {
            case C.TYPE_DASH:
                return new DashMediaSource.Factory(dataSourceFactory).setLivePresentationDelayMs(0, true).createMediaSource(uri);
            case C.TYPE_SS:
